...
-- 											 FROM VMatch
-- 											 WHERE tournamentId = 1 AND bracketLevel = (SELECT maxBracketLevel FROM Tournament where TournamentId = '1'))));
...
-- CREATE VIEW matchesInNA AS (SELECT matchId FROM VMatch RIGHT JOIN tournamentsNA ON  VMatch.tournamentId =tournamentsNa.tournamentId);
-- DROP VIEW teamsInNA;
 -- CREATE VIEW teamsInNA AS (SELECT DISTINCT(teamId) FROM Roster RIGHT JOIN matchesInNA ON Roster.matchId = matchesInNA.matchId);
-- CREATE VIEW teamInfo AS (SELECT Team.tName, Team.tWins FROM Team  RIGHT JOIN teamsInNA ON Team.teamId = teamsInNA.teamId);
